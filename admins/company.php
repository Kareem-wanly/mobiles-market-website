...
if (empty($_SESSION["userID"])) {
    ...
    if ($_SESSION["accountType"] == 2) {
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            if ($row['product_condition'] == 1) {
                                ...
                                <div class="product-card">
                                    ...
                                    <div class="bottom">
                                        <a href="?controls=productInactive&productID=<?php echo $row['productID'] ?>&companyID=<?php echo $companyID ?>">inactive</a>
                                        <a href="product.php?productID=<?php echo $row['productID'] ?>">view details</a>
                                    </div>
                                </div>
                                <?php
                            } elseif($row['product_condition'] == 2) {
                                ?>
                                    <div class="product-card">
                                        <div class="top">
                                            <img src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($row['mainPic']) ?>" alt="">
                                        ...
                                        <div class="bottom">
                                            <a href="?controls=productActive&productID=<?php echo $row['productID'] ?>&companyID=<?php echo $companyID ?>">active</a>
                                            <a href="product.php?productID=<?php echo $row['productID'] ?>">view details</a>
                                        </div>
                                    </div>
                                <?php
        ...
        } elseif($controls == 'productInactive') {
            $productID = $_GET['productID'];
            $companyID = $_GET['companyID'];
            if (product_condition(2, $productID) == 1) {
                header('REFRESH:0;URL=company.php?companyID=' .  $companyID);
            }
        } elseif($controls == 'productActive') {
            $productID = $_GET['productID'];
            $companyID = $_GET['companyID'];
            if (product_condition(1, $productID) == 1) {
                header('REFRESH:0;URL=company.php?companyID=' .  $companyID);
            }
        }
